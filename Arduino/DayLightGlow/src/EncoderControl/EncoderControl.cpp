...
EncoderControl::EncoderControl(uint8_t CLK, uint8_t DT, uint8_t SW) : 
  encoder(new Encoder(CLK, DT)), button_pin(SW)
{
  ;
}
...
void EncoderControl::update(uint32_t cycleTime)
{
...
void EncoderControl::setPosition(int32_t position)
{
...
{
  return state_up || state_down || state_left || state_right || state_short || state_long;
}
